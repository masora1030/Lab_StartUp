...
class CNNNet(nn.Module):
    ...
    def forward(self, x): # predictに相当(順伝搬)
        x = F.relu(self.conv1(x))                # Drop Out なし 1
        # x = self.dropout1(F.relu(self.conv1(x))) # Drop Out あり 1
        x = self.pool(F.relu(self.conv2(x)))                # Drop Out なし 2
        # x = self.pool(self.dropout2(F.relu(self.conv2(x)))) # Drop Out あり 2
        x = F.relu(self.conv3(x))                # Drop Out なし 3
        # x = self.dropout3(F.relu(self.conv3(x))) # Drop Out あり 3
        x = F.relu(self.conv4(x))                # Drop Out なし 4
        # x = self.dropout4(F.relu(self.conv4(x))) # Drop Out あり 4
        x = F.relu(self.conv5(x))                # Drop Out なし 5
        # x = self.dropout5(F.relu(self.conv5(x))) # Drop Out あり 5
        x = F.relu(self.conv6(x))                # Drop Out なし 6
        # x = self.dropout6(F.relu(self.conv6(x))) # Drop Out あり 6
        x = x.view(-1, 6*6*256)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x

class CNN1Net(nn.Module):
    def __init__(self):
...
class CNN3Net(nn.Module):
    def __init__(self):
        super(CNN3Net, self).__init__()
        # xAT + b (重みとバイアスを足し合わせる作業と、重みパラメータを保持する機能)
        # Conv2d(in_channels, out_channels(), kernel_size(フィルターサイズ), stride=1, padding=0,
        # dilation=1, groups=1, bias=True, padding_mode='zeros')
        self.conv1 = nn.Conv2d(3, 64, 3) # 32x32x3 -> 30x30x64
...
class CNN4Net(nn.Module):
    def __init__(self):
        super(CNN4Net, self).__init__()
        # xAT + b (重みとバイアスを足し合わせる作業と、重みパラメータを保持する機能)
        # Conv2d(in_channels, out_channels(), kernel_size(フィルターサイズ), stride=1, padding=0,
        # dilation=1, groups=1, bias=True, padding_mode='zeros')
        self.conv1 = nn.Conv2d(3, 64, 3) # 32x32x3 -> 30x30x64
...
class CNN5Net(nn.Module):
    def __init__(self):
        super(CNN5Net, self).__init__()
        # xAT + b (重みとバイアスを足し合わせる作業と、重みパラメータを保持する機能)
        # Conv2d(in_channels, out_channels(), kernel_size(フィルターサイズ), stride=1, padding=0,
        # dilation=1, groups=1, bias=True, padding_mode='zeros')
        self.conv1 = nn.Conv2d(3, 64, 3) # 32x32x3 -> 30x30x64
...
class CNN6Net(nn.Module):
    def __init__(self):
        super(CNN6Net, self).__init__()
        # xAT + b (重みとバイアスを足し合わせる作業と、重みパラメータを保持する機能)
        # Conv2d(in_channels, out_channels(), kernel_size(フィルターサイズ), stride=1, padding=0,
        # dilation=1, groups=1, bias=True, padding_mode='zeros')
        self.conv1 = nn.Conv2d(3, 64, 3) # 32x32x3 -> 30x30x64
...
class CNN7Net(nn.Module):
    def __init__(self):
        super(CNN7Net, self).__init__()
        # xAT + b (重みとバイアスを足し合わせる作業と、重みパラメータを保持する機能)
        # Conv2d(in_channels, out_channels(), kernel_size(フィルターサイズ), stride=1, padding=0,
        # dilation=1, groups=1, bias=True, padding_mode='zeros')
        self.conv1 = nn.Conv2d(3, 64, 3) # 32x32x3 -> 30x30x64
...
Optims = [optKind1, optKind2, optKind3, optKind4, optKind5, optKind6, optKind7, optKind8]
Optimizers = [optimizer1, optimizer2, optimizer3, optimizer4, optimizer5, optimizer6, optimizer7, optimizer8]
networks = [network1, network2, network3, network4, network5, network6, network7, network8]
