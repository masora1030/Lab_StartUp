...
# load CIFAR10 data
train_Dataset1 = dsets.CIFAR10(  # CIFAR10 default dataset
    root='./data_cifar11/',  # rootで指定したフォルダーを作成して生データを展開。これは必須。
    train=True,  # 学習かテストかの選択。これは学習用のデータセット
    transform=form,  # 正規化
    # transform=transforms.ToTensor(),  # Pytroch のテンソルに変換する  # 自動で形式を変換するのでラベルをOne_hotにする必要等がない。
    download=True)  # ダウンロードするかどうかの選択(当然する) ここには正解ラベルのデータと入力文字のデータの対がまとめて入っている。.

test_dataset1 = dsets.CIFAR10(
    root='./data_cifar11/',
    train=False, # 学習かテストかの選択。これはテスト用のデータセット
    transform=form,  # 正規化
    # transform=transforms.ToTensor(),  # Pytroch のテンソルに変換する。 # 自動で形式を変換するのでラベルをOne_hotにする必要等がない。
...
train_dataset1, valid_dataset1 = torch.utils.data.random_split(  # データセットの分割(validation)
    train_Dataset1,  # 分割するデータセット(さっきダウンロードしたやつ)
    [40000, 10000])  # 分割数    (50000個の学習用データを、さらに40000個の学習用データと10000個のバリデーション用データに分割)

print('train_dataset1 = ', len(train_dataset1)) # 40000
print('valid_dataset1 = ', len(valid_dataset1)) # 10000
print('test_dataset1 = ', len(test_dataset1)) # 10000

# set data loader
# データセットからミニバッチ単位でデータを取り出し、ネットワークへ供給するためのローダー(バッチ処理の準備)
# バッチサイズを変更すると、勾配のノイズ(不安定差)が変わる。
train_loader1 = torch.utils.data.DataLoader(
    dataset=train_dataset1,  # データセットの指定
    batch_size=64,  # ミニバッチの指定
    shuffle=True,  # シャッフルするかどうかの指定
    num_workers=2)  # コアの数
...
optKind2 = 'DataNormalization'

times, test_acc_list, train_loss_lists, train_acc_lists, val_loss_lists, val_acc_lists = [], [], [], [], [], []
Optims = [optKind1, optKind2]
Optimizers = [optimizer1, optimizer2]
networks = [network1, network2]

train_loaders = [train_loader, train_loader1]
valid_loaders = [valid_loader, valid_loader1]
test_loaders = [test_loader, test_loader1]
...
for i in range(len(Optims)):
    ...
    for epoch in range(num_epochs):
        ...
        # 学習する
        network.train()
        for images, labels in train_loaders[i]:  # ミニバッチ回数実行
        ...
        avg_train_loss = train_loss / len(train_loaders[i].dataset)  # lossの平均を計算
        avg_train_acc = train_acc / len(train_loaders[i].dataset)  # accの平均を計算

        # ======== valid_mode ======
        # 評価する
        network.eval()
        with torch.no_grad():  # 必要のない計算を停止
            for images, labels in valid_loaders[i]:
                images, labels = images.to(device), labels.to(device)  # そのまま使う
        ...
        avg_val_loss = val_loss / len(valid_loaders[i].dataset)  # lossの平均を計算
        avg_val_acc = val_acc / len(valid_loaders[i].dataset)  # accの平均を計算
...
plt.xlabel('epoch')
plt.ylabel('loss')
plt.title('CNN Training and validation loss with Data Normalization')
plt.grid()
plt.savefig('loss{}_by_{}.png'.format('CNN', 'various_Data_Normalization'), bbox_inches="tight")
...
plt.xlabel('epoch')
plt.ylabel('acc')
plt.title('CNN Training and validation acc with Data Normalization')
plt.grid()
plt.savefig('acc{}_by_{}.png'.format('CNN', 'various_Data_Normalization'), bbox_inches="tight")
