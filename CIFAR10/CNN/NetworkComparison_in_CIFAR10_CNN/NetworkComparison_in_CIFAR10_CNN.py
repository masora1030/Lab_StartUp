...
class FewCNNNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 64, 3) # 32x32x3 -> 30x30x64
        self.bn1 = nn.BatchNorm2d(64)  # Batch Normalization
        self.pool = nn.MaxPool2d(2, 2)  # 30x30x64 -> 15x15x64
        self.dropout1 = nn.Dropout2d(0.2)
        self.conv2 = nn.Conv2d(64, 128, 3)  # 15x15x64 -> 13x13x128
        self.bn2 = nn.BatchNorm2d(128)  # Batch Normalization
        self.fc1 = nn.Linear(13*13*128, 1024)
        self.fc2 = nn.Linear(1024, 10)
...
for i in range(len(Optims)):
    ...
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.title('CIFAR10 CNN Training and validation loss with {}'.format(Optims[i]))
    plt.grid()
...
for i in range(len(Optims)):
    ...
    plt.xlabel('epoch')
    plt.ylabel('acc')
    plt.title('CIFAR10 CNN Training and validation acc with {}'.format(Optims[i]))
    plt.grid()
...
plt.xlabel('epoch')
plt.ylabel('loss')
plt.title('CIFAR10 CNN Training and validation loss with various networks')
plt.grid()
...
plt.xlabel('epoch')
plt.ylabel('acc')
plt.title('CIFAR10 CNN Training and validation acc with various networks')
plt.grid()
