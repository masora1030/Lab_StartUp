...
# save weights
torch.save(network.state_dict(), 'cifer10_net.ckpt')
...
plt.xlabel('epoch')
plt.ylabel('loss')
plt.title('MLP Training and validation loss by {} (Many Layers)'.format(optKind))
plt.grid()
plt.savefig('loss{}.png'.format('MLP_manyLayers'))
...
plt.xlabel('epoch')
plt.ylabel('acc')
plt.title('MLP Training and validation accuracy by {} (Many Layers)'.format(optKind))
plt.grid()
plt.savefig('acc{}.png'.format('MLP_manyLayers'))
