...
class ManyMLPNet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(28*28*1, 784) # 784の配列を784に線形変換(入力) (in, out)
        nn.init.kaiming_normal_(self.fc1.weight) # reluなので、Heの初期値に重みを初期化
        self.bn1 = nn.BatchNorm1d(784)
        self.fc2 = nn.Linear(784, 784) # 784の配列を784に線形変換
        nn.init.kaiming_normal_(self.fc2.weight)
        self.bn2 = nn.BatchNorm1d(784)
        self.fc3 = nn.Linear(784, 784) # 784の配列を784に線形変換
        nn.init.kaiming_normal_(self.fc3.weight)
        self.bn3 = nn.BatchNorm1d(784)
        self.fc4 = nn.Linear(784, 784) # 784の配列を784に線形変換
        nn.init.kaiming_normal_(self.fc4.weight)
        self.bn4 = nn.BatchNorm1d(784)
        self.fc5 = nn.Linear(784, 10) # 784の配列を10に線形変換(出力)
...
class FewMLPNet(nn.Module):
    def __init__(self):
        ...
        nn.init.kaiming_normal_(self.fc2.weight)
        self.bn2 = nn.BatchNorm1d(784)
        self.fc3 = nn.Linear(784, 10) # 784の配列を10に線形変換(出力)
        nn.init.kaiming_normal_(self.fc3.weight)
        self.dropout1 = nn.Dropout2d(0.2)
        self.dropout2 = nn.Dropout2d(0.2)
...
for i in range(len(Optims)):
    ...
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.title('MNIST MLP Training and validation loss with {}'.format(Optims[i]))
    plt.grid()
...
for i in range(len(Optims)):
    ...
    plt.xlabel('epoch')
    plt.ylabel('acc')
    plt.title('MNIST MLP Training and validation acc with {}'.format(Optims[i]))
    plt.grid()
...
plt.xlabel('epoch')
plt.ylabel('loss')
plt.title('MNIST MLP Training and validation loss with various networks')
plt.grid()
...
plt.xlabel('epoch')
plt.ylabel('acc')
plt.title('MNIST MLP Training and validation acc with various networks')
plt.grid()
