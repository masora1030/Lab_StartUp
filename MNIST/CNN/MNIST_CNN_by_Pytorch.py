...
# CNN
# CNNモデルの構築。今回は適当に4層。(Conv Relu Pooling | Conv Relu (dropout) | Affine relu (dropout) | Affine (Softmax))
# nn.Moduleを継承している。
...
plt.xlabel('epoch')
plt.ylabel('loss')
plt.title('CNN Training and validation loss by {}'.format(optKind))
plt.grid()
plt.savefig('loss{}.png'.format('CNN'))
...
plt.xlabel('epoch')
plt.ylabel('acc')
plt.title('CNN Training and validation accuracy by {}'.format(optKind))
plt.grid()
plt.savefig('acc{}.png'.format('CNN'))