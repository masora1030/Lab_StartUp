...
network1 = VGG16().to(device) # networkにさっき定義したnetworkを代入
optimizer1 = optim.SGD(network1.parameters(), lr=0.01, weight_decay=5e-4) # optimizrにSGDを指定
optKind1 = 'SGD'

network2 = VGG16().to(device) # networkにさっき定義したnetworkを代入
...
for i in range(len(Optims)):
    ...
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.title('MNIST CNN Training and validation loss by {} (VGG16)'.format(Optims[i]))
    plt.grid()
    plt.savefig('loss{}_by_{}.png'.format('VGG16', Optims[i]))
...
for i in range(len(Optims)):
    ...
    plt.xlabel('epoch')
    plt.ylabel('acc')
    plt.title('MNIST CNN Training and validation acc by {} (VGG16)'.format(Optims[i]))
    plt.grid()
    plt.savefig('acc{}_by_{}.png'.format('VGG16', Optims[i]))
...
plt.xlabel('epoch')
plt.ylabel('loss')
plt.title('MNIST CNN Training and validation loss by various optimizers')
plt.grid()
...
plt.xlabel('epoch')
plt.ylabel('acc')
plt.title('MNIST CNN Training and validation acc by various optimizers')
plt.grid()
